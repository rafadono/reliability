...
class TestJackknifeAnalysis:
    ...
    def test_multi_parameter_ci(self):
        ...
        data = {
            "weibull_alpha": np.array(
                [100, 110, 120, 130, 140, 150, 160, 170, 180, 190]
            ),
            "weibull_beta": np.array(
                [1.5, 1.6, 1.5, 1.7, 1.6, 1.5, 1.8, 1.7, 1.6, 1.5]
            ),
        }
        ...
        assert all(
            "mean" in result[key] and "ci_lower" in result[key] for key in result
        )
    ...
    def test_constant_data_ci(self):
        ...
        assert (
            ...
            or abs(result["ci_upper"] - result["ci_lower"]) < 1
        )
