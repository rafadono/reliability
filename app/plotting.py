...
def plot_metric_plotly(best_fit, tiempo_max, metric_func, title, y_label):
...
def streamlit_app()-> None:
    ...
    if uploaded_file is not None:
        ...
        if fit_tbx and fit_ttx:
            ...
            fig_tbx_conf = plot_confiabilidad(fit_tbx, tiempo_max)
            fig_tbx_hf   = plot_tasa_de_falla(fit_tbx, tiempo_max)

            fig_ttx_pdf  = plot_pdf(fit_ttx, tiempo_max)
            fig_ttx_cdf  = plot_cdf(fit_ttx, tiempo_max)

            st.plotly_chart(fig_tbx_conf, use_container_width=True)
            st.plotly_chart(fig_tbx_hf, use_container_width=True)
