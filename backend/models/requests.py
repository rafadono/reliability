...
class OptimalPMRequest(WeibullFitRequest):
    ...
    cost_failure: float = Field(..., description="Cost of a single Corrective Maintenance action (failure)")

class ConditionalReliabilityRequest(WeibullFitRequest):
    current_age: float = Field(..., description="Current age of the component since last major overhaul or PM")
    mission_time: float = Field(..., description="The duration of the future mission to evaluate")