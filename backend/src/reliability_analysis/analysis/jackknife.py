...
def calculate_jackknife_ci(
    ...
    for i in range(n):
        ...
        try:
            stat = (
                statistic_func(data_loo, cens_loo)
                if censored is not None
                else statistic_func(data_loo)
            )
...
def calculate_multi_parameter_ci(
    ...
    if fit_func is None or param_names is None:
        raise ValueError(
            "fit_func and param_names are required when data is not a dict"
        )
    ...
    for i in range(n):
        ...
        try:
            params = (
                fit_func(data_loo, cens_loo)
                if censored is not None
                else fit_func(data_loo)
            )
...
def sensitivity_analysis(
    data: np.ndarray, fit_func, param_name: str, confidence: float = 0.95
) -> Dict[str, Any]:
    """
