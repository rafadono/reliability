...
from api.analysis import compute_criticality, compute_bad_actors, compute_event_plot, compute_ram_simulation
...
def execute_filter_node(node: NodeSchema, inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    filtered_df = df.copy()
    if equipment:
        if "Equipment" in filtered_df.columns and (filtered_df["Equipment"] == equipment).any():
    ...
    if failure_type:
        if isinstance(failure_type, list):
            if len(failure_type) > 0 and "Type" in filtered_df.columns:
                valid_types = [t for t in failure_type if (filtered_df["Type"] == t).any()]
                if valid_types:
                    filtered_df = filtered_df[filtered_df["Type"].isin(valid_types)]
                else:
                    node.data["type"] = []
        elif "Type" in filtered_df.columns and (filtered_df["Type"] == failure_type).any():
...
def execute_kijima_node(node: NodeSchema, inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "ap": first.get("ap"),
            "r2": first.get("r2"),
            "applied_config": {"model_types": model_types, "min_tbx": min_tbx, "min_ttx": min_ttx}
        }
    except Exception as e:
...
def execute_criticality_node(node: NodeSchema, inputs: Dict[str, Any]) -> Dict[str, Any]:
    df = inputs.get("df")
    if df is None:
...
def execute_rcm_node(node: NodeSchema, inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not equipment:
        return {"error": "Selecciona un equipo (o filtra por un único equipo aguas arriba) para generar las fichas RCM."}
...
def execute_rca_node(node: NodeSchema, inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    equipment = resolve_node_equipment(node, df)
    if not equipment:
        return {"error": "Selecciona un equipo (o filtra por un único equipo aguas arriba) para generar el análisis RCA."}
...
_LEGACY_CATEGORY_KEYWORDS = [
    ("Operational", ["operacional", "operación", "decision", "decisión", "operational", "operation", "process", "operator"]),
    ("Cleaning/Blockage", ["limpieza", "atollo", "obstrucción", "obstruido", "cleaning", "blockage", "jam", "clog", "obstructed"]),
    ("Mechanical", ["mecánico", "mecanico", "perno", "shaft", "eje", "rodamiento", "bearing", "correa", "motor", "mechanical", "bolt", "belt"]),
    ("Electrical", ["eléctrico", "electrico", "cable", "bobina", "fase", "breaker", "contacto", "potencia", "electrical", "coil", "phase", "contact", "power"]),
    ("Instrumentation/Failure", ["falla", "alarma", "sensor", "calibracion", "calibración", "instrumentación", "instrumento", "failure", "alarm", "calibration", "instrumentation", "instrument"]),
]


def execute_comment_mining_node(node: NodeSchema, inputs: Dict[str, Any]) -> Dict[str, Any]:
    df = inputs.get("df")
    if df is None or df.empty:
        return {"coverage": 0.0, "total_comments": 0, "categories": [], "model": "Legacy Keyword NLP"}

    comment_col = next((c for c in df.columns if c.lower() in ("comentario", "comment")), None)
    if not comment_col:
        return {"error": "El dataset no contiene una columna de comentarios."}

    valid = df.dropna(subset=[comment_col])
    valid = valid[~valid[comment_col].astype(str).str.lower().isin(["---", "nan", "none", "null", "no aplica", "n/a"])]
    ...
    for _, row in valid.iterrows():
        ...
        for cat_name, keywords in _LEGACY_CATEGORY_KEYWORDS:
            if any(k in text_lower for k in keywords):
                category = cat_name
                break
        records.append({"category": category, "type": str(row.get(type_col, "Unknown")) if type_col else "Unknown"})
    ...
    for cat_name in [c for c, _ in _LEGACY_CATEGORY_KEYWORDS] + ["Others"]:
        ...
        categories.append({"category": cat_name, "count": len(cat_records), "top_types": top_types})
...
async def execute_pipeline(req: PipelineExecuteRequest) -> Dict[str, Any]:
    ...
    for node in execution_order:
        ...
        if len(dfs_to_combine) == 1:
            ...
            try:
                combined_df = pd.concat(dfs_to_combine, ignore_index=True).drop_duplicates()
                node_inputs = {"df": combined_df}
            except Exception as concat_err:
                logger.warning(f"Could not concat DataFrames for node {node_id}: {concat_err}")
                node_inputs = {"df": dfs_to_combine[0]}
        ...
        try:
            if node_type == "dataSource":
                ...
                results[node_id] = {"status": "success", "output": res["ui_data"], "execution_ms": elapsed}
                ...
                elapsed = round((time.time() - start_time) * 1000, 2)
                results[node_id] = {"status": "success", "output": res, "execution_ms": elapsed}
