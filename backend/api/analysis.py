...
async def jackknife_analysis(req: AnalysisRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        scatter_data = [{
            ...
            "x_prob": float(row['failures']) / total_failures if total_failures > 0 else 0.0,
        ...
        return {
            ...
            "averages": {"failures": avg_failures, "probability": avg_prob, "total_downtime": avg_total, "avg_downtime": avg_mean}
        }
    except Exception as e:
...
async def fit_data(req: WeibullFitRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        results = fitter.fit_weibull(column=target_col, censored_failure_types=req.censored_failure_types)
        
        if not results or results.get('beta') is None:
            return {"status": "error", "message": "Could not fit Weibull distribution", "parameters": None}
        ...
        try:
            ...
            if results.get('beta') and results.get('eta'):
                ...
                t_max = float(fit_data['TBX'].max()) if 'TBX' in fit_data.columns else 1000.0
        ...
        return {
            "status": "success",
            "parameters": {"beta": float(results['beta']), "eta": float(results['eta'])},
            "goodness_of_fit": {"aic": float(results.get('aic')), "bic": float(results.get('bic'))},
...
async def bad_actors_analysis(req: AnalysisRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        for name, group in data.groupby(group_col):
            ...
            availability = (uptime / (uptime + downtime)) * 100 if (uptime + downtime) > 0 else 0.0
            
            stats.append({
                "name": str(name), "failures": failures, "downtime": downtime,
                "mttr": mttr, "mtbf": mtbf, "availability": availability
            })
...
async def reliability_growth(req: AnalysisRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        if uptime_col and uptime_col in data.columns:
            ...
            return {"status": "success", "cumulative_time": cum_time, "cumulative_failures": cum_failures}
...
async def optimal_pm(req: OptimalPMRequest) -> Dict[str, Any]:
    try:
        ...
        if not params or not params.get("beta"):
            raise HTTPException(status_code=400, detail="Could not fit Weibull to calculate Optimal PM.")
        ...
        import math
        if optimal_time is None or math.isnan(float(optimal_time)) or math.isinf(float(optimal_time)):
            opt_val = None
        else:
...
async def conditional_reliability(req: ConditionalReliabilityRequest) -> Dict[str, Any]:
    try:
        ...
        params = fit_response.get("parameters")
        if not params or not params.get("beta"):
            raise HTTPException(status_code=400, detail="Could not fit Weibull to calculate conditional reliability.")
...
async def comment_mining(req: AnalysisRequest) -> Dict[str, Any]:
    ...
    try:
        data = state.filter_manager.get_filtered_data()
        if data.empty:
            return {"status": "success", "keywords": [], "categories": [], "coverage": 0.0}
        ...
        generic_words = {
            'decision', 'decisión', 'operacional', 'operacion', 'operación', 'detencion', 'detención', 
            'detenida', 'detenido', 'detencin', 'proceso', 'procesos', 'sistema', 'sistemas', 'falla', 
            'fallas', 'parada', 'paradas', 'bajo', 'alto', 'nivel', 'niveles', 'vacio', 'vacío', 
            'corriendo', 'tiempo', 'tiempos', 'minuto', 'minutos', 'hora', 'horas', 'días', 'día', 
            'sección', 'área', 'operario', 'operarios', 'turno', 'turnos', 'reporte', 'reportes', 
